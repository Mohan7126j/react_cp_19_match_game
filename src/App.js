...
const Tab = props => {
  ...
  return (
    <li className="tabItem tab ">
      <button
        className={`btn-tab ${isActive ? 'selected-tab' : ''}`}
        type="button"
...
const Images = props => {
  ...
  return (
    <li>
      <button className="btn-tab" type="button" onClick={onClickHandle}>
        <img className="images" src={thumbnailUrl} alt="thumbnail" />
      </button>
    </li>
  )
}
...
class App extends Component {
  ...
  startTimer = () => {
    ...
    const {second, isPlaying} = this.state
  ...
  generate = () => {
    ...
    const imageDetails = imagesList[randomNumber - 1]
    this.setState({
  ...
  render() {
    const {selectedTabId, second, currentImage, score, isPlaying} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {isPlaying ? (
            ...
            <ul className="tabItem">
              {tabsList.map(eachItem => (
                <Tab
                  ...
                  isActive={eachItem.tabId === selectedTabId}
                />
              ))}
            </ul>
            <ul className="img-container">
              {imagesList.map(eachItem =>
                eachItem.category === selectedTabId ? (
                  <Images
                    key={eachItem.id}
          ...
          <Result score={score} reset={this.reset} />
        )}
      </div>
    )
  }
